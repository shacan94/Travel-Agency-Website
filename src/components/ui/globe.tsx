...
/**
 * Earth globe — supplied component from ruixen.ui.
 *
 * A CSS-only rotating planet rendered from a single background image whose
 * background-position animates horizontally, with a stack of inset shadows
 * baked into the box-shadow to fake the day/night terminator lighting.
 * Twinkling stars are layered as absolutely positioned 1×1 dots.
 *
 * Ported as-supplied. The only adjustment vs the source is removing the
 * outer `<div className="flex items-center justify-center h-screen">`
 * demo wrapper — that wrapper is appropriate for a stand-alone demo page
 * but would force any embedding container (e.g. the Hero right column)
 * to be a full viewport tall. Centering is now the caller's responsibility.
 *
 * @keyframes are declared via an inline <style> element (NOT styled-jsx,
 * which would scope them and break the animation references).
 */
